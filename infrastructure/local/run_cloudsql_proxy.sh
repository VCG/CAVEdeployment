...
export INSTANCE_CONNECTION_NAME=$SQL_PROJECT_NAME:$REGION:$SQL_INSTANCE_NAME

docker pull gcr.io/cloudsql-docker/gce-proxy:1.33.6
...
-v $HOME/.config/gcloud/:/config \
  ...
  --name gce-proxy \
  gcr.io/cloudsql-docker/gce-proxy:1.33.6 /cloud_sql_proxy \
